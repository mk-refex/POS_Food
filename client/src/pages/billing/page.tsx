...
export default function Billing() {
  ...
  // Debounced server-side search for guests
  useEffect(() => {
    const handle = setTimeout(async () => {
      try {
        const res = await mastersApi.getGuests({
          q: guestSearch,
          limit: 50,
          sortBy: "name",
        });
  ...
  const handlePrintBill = async () => {
    ...
    if (isGuest) {
      customerData =
        selectedGuestObj ||
        guests.find((g) => g.id.toString() === selectedGuest);
                    ...
                    <div className="relative">
                      <select
                        ...
                        onChange={(e) => {
                          ...
                          setSelectedGuest(id);
                          const g =
                            guests.find((x) => x.id.toString() === id) || null;
